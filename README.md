...
แยกที่เก็บตามลักษณะข้อมูล:

- **`sessionStorage['se_cache']`** — เก็บ meta (`date_from`/`date_to`/`report_type`/`saved_at`) + pre-aggregated stats (`page2_stats`, `page3_stats`) ขนาดเล็ก (2-30 KB) เข้าถึง sync
- **IndexedDB `se_dashboard.cache['records']`** — เก็บ records ที่ slim เหลือ 11 ฟิลด์ ขนาดใหญ่ (10+ MB ที่ 1-year fetch) ไม่ติด quota 5MB ของ sessionStorage

Flow:
- Fetch เสร็จ → `saveCache()` เซฟ meta+stats sync ลง sessionStorage และ fire-and-forget เซฟ records ลง IDB
- กลับหน้า 1 → `restoreCache()` async อ่าน meta sync (form fields populate ทันที) แล้ว await records จาก IDB → re-render dashboard cards + supervisor table
- เปิด /page2, /page3 → ใช้ `page2_stats`/`page3_stats` จาก sessionStorage โดยไม่ต้องโหลด records (เร็ว, sync)
- เปลี่ยน report_type หรือ fetch ใหม่ → `clearCache()` ลบทั้ง sessionStorage และ IDB
- เก็บ snapshot ของ `date_from`/`date_to`/`report_type` จาก FormData ตอน submit (กัน race condition)

Benchmark IDB roundtrip ที่ 200k records (fake-indexeddb): put 132ms · get 143ms
...
- **ดึง 1 ปี (~200k records):** หลัง parallel + field strip ใช้เวลา ~6-7 นาที, browser peak memory ~60-80 MB, records เก็บใน IndexedDB (ไม่ติด sessionStorage quota) → /page1, /page2, /page3 ทำงานครบ ทั้ง first fetch และหลัง refresh/navigate กลับ
